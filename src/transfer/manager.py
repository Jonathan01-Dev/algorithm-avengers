...
class TransferManager:
        ...
        try:
            ...
            if file_id in self.local_files:
                ...
                if session:
                    ...
                    nonce, ciphertext, tag = encrypt_aes_gcm(
                        session.session_key, chunk_data
                    )
                    ...
                    print(
                        f"[DEBUG] No session found for requester {requester_id_hex[:8]}, sending unencrypted chunk"
                    )
                    response_payload = {
                        "file_id": file_id,
                        "chunk_idx": chunk_idx,
                        "nonce": "00" * 12,
                        "ciphertext": chunk_data.hex(),
                        "tag": "00" * 16,
                        "chunk_hash": hashlib.sha256(chunk_data).hexdigest(),
                        "signature": signature.hex(),
                        "unencrypted": True,
                ...
                print(
                    f"[DEBUG] File {file_id[:8]} not in local_files. Have: {list(self.local_files.keys())}"
                )
    ...
    def download_file(self, manifest, peers):
        ...
        for i, (peer_id, peer_data) in enumerate(peers.items()):
            ...
            if start_idx >= nb_chunks:
                break

            t = threading.Thread(
                target=self._download_worker,
                args=(peer_id, peer_data, manifest, range(start_idx, end_idx)),
            )
            threads.append(t)
            t.start()
    ...
    def _request_chunk(self, peer_id, peer_data, file_id, chunk_idx):
        from src.network.server import recv_packet as _recv_packet

        # 1. Ensure session exists
        if peer_id not in self.messaging_manager.sessions:
            print(f"[DEBUG] No session for {peer_id[:8]}, initiating handshake...")
            if not self.messaging_manager.initiate_handshake(
                peer_data["ip"], peer_data["tcp_port"], peer_id
            ):
        ...
        try:
            with socket.create_connection(
                (peer_data["ip"], peer_data["tcp_port"]), timeout=10
            ) as sock:
                ...
                resp_pkt = _recv_packet(sock, timeout=30)
                if not resp_pkt:
                    print(f"[DEBUG] No data received for chunk {chunk_idx}")
                    return False
                if resp_pkt.type != TYPE_CHUNK_DATA:
                    print(
                        f"[DEBUG] Wrong packet type for chunk {chunk_idx}: {resp_pkt.type}"
                    )
                    return False
                ...
                if resp_payload["unencrypted"]:
                    print(f"[DEBUG] Received unencrypted chunk for chunk {chunk_idx}")
                    chunk_data = bytes.fromhex(resp_payload["ciphertext"])
                else:
                    chunk_data = decrypt_aes_gcm(
                        session.session_key, nonce, ciphertext, tag
                    )
                ...
                # Verify SHA-256
                expected_hash = self.active_downloads[file_id]["manifest"]["chunks"][
                    chunk_idx
                ]["hash"]
